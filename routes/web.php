...
Route::get('about-us/showd/{emailw}', function($emailw){
    $result = app()->make('App\Http\Controllers\AboutUsController')->showd($emailw);
    return $result;
})->name('aboutus.showd');


Route::get('/load-more-products', 'Front\FrontendController@loadMoreProducts')->name('load.more.products');

Route::get('/fffff',function(){
    return view('ff');
})->name('ffd');
Route::post('/submit-ticket', 'User\UserBaseController@submitTicket')->name('submitTicketd');
Route::post('/send-message', 'User\UserBaseController@sendMessage')->name('sendMessage');
Route::get('/gg',[SubController::class,'index']);
Route::post('/subcategory/create', function() {return 'mass';} )->name('admin-subcat-mass');

Route::get('color-self', 'Front\VendorController@vendorColorForm')->name('vendor.color.form');
Route::post('color', 'Front\VendorController@updateVendorColor')->name('vendor.background.update');


Route::post('/generate-description', 'ProductDescriptionController@generateDescription');
Route::post('/remove-background', 'ImageBackgroundController@removeBackground');
Route::post('/remove-background-without-product', 'ImageBackgroundController@postRemoveBackground');
Route::get('/about-us', 'Front\FrontendController@aboutUs')->name('about-us');
...
Route::prefix('admin')->group(function () {
    ...
    Route::get('/get-states/{countryId}', function($countryId) {
        ...
        $options = '<option value="">اختر الولاية</option>';
    ...
    Route::get('/get-cities', function(Request $request) {
        ...
        $options = '<option value="">اختر المدينة</option>';
        foreach ($cities as $city) {
            $options .= '<option value="' . $city->id . '">' . $city->city_name . '</option>';
        }
        return response()->json($options);
    })->name('state.wise.city');
    ...
    Route::group(['middleware' => 'permissions:vendors'], function () {
        Route::get('/referrals/list/{referralCode}', [ReferralController::class, 'list'])->name('referrals.list');
        ...
        Route::post('/admin/update-verification-status', [VendorController::class, 'updateVerificationStatus'])->name('admin.update-verification-status');
...
Route::group(['middleware' => 'maintenance'], function () {
    ...
    Route::prefix('vendor')->group(function () {
        ...
        Route::group(['middleware' => 'vendor'], function () {
             ...
             Route::get('about-us/show', [AboutUsController::class, 'show'])->name('aboutus.show');
            // VENDOR DASHBOARD

            Route::get('/dashboard', 'Vendor\VendorController@index')->name('vendor.dashboard');
            Route::get('/about-us', 'Vendor\VendorController@index')->name('vendor.aboutus');
    ...
    Route::prefix('user')->group(function () {
        ...
        Route::get('/package', 'SubController@packages')->name('user-package');

        Route::get('/package', 'User\SubscriptionController@package')->name('user-package');
        Route::get('/packages', 'User\SubscriptionController@packages')->name('user-packages');
    ...
    Route::post('/contactw', 'Front\FrontendController@contactadminemail')->name('front.contactemail');
    
    //contact-vender
    Route::get('/contactvender', 'Front\FrontendController@contactvender')->name('front.contactvender');
    Route::post('/contactvenderew', 'Front\FrontendController@contactvenderemail')->name('front.contactvenderemail');
    ...
    Route::get('/category/{subcategoryId?}','Front\CatalogController@category')->name('front.subcategory');
    // ahmad
    ...
    Route::get('/checkout/payment/{slug1}/{slug2}', function(){return "d";})->name('front.load.payment');
    ...
    Route::post('/save-packing', 'CheckoutController@savePacking')->name('save.packing');
    ...
    // Route::get('auth/facebook', 'Auth\User\SocialRegisterController@redirectToFacebook')->name('social-facebook');
    // Route::get('auth/facebook/callback', 'Auth\User\SocialRegisterController@handleFacebookCallback');
