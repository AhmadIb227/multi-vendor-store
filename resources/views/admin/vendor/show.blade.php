...
@media (max-width: 768px) {
  ...
  table {
    width: 100%; /* للتأكد من أن الجدول يتوسع ليتناسب مع العرض */
  }
}
  ...
  <div class="table-responsive show-table">
    <table class="table">
                                                        ...
                                                        <tr>
                                                            <th>View All Referrals</th>
                                                             <td> 
                                                                <a href="{{ route('referrals.list', ['referralCode' => $data->referral_code]) }}" class="btn btn-primary">
                                                                View All 
                                                             </a></td>
                                                        ...
                                                        <tr>
                                                            <th>referral_link: </th>
                                                            <td>{{ $data->referral_link }}</td>
                                                        </tr>
                                                        <tr>
                                                            <th>referral code points_vendor: </th>
                                                            <td>{{ $data->points_vendor }}</td>
                                                        </tr>
                                                        <tr>
                                                            <th>referral code points_user: </th>
                                                            <td>{{ $data->points_user }}</td>
                                                        </tr>
                                                        <tr>
                                                            <th>birthdate:</th>
...
width: 60px; /* زيادة عرض الزر */
height: 30px; /* زيادة ارتفاع الزر إذا كنت ترغب */
}
...
position: absolute;
content: "";
height: 26px; /* ارتفاع الدائرة داخل الزر */
width: 26px;  /* عرض الدائرة داخل الزر */
left: 2px;
bottom: 2px;
...
transform: translateX(20px); /* تحريك الدائرة داخل الزر عند التبديل */
}
 ...
 <div class="col-lg-4 d-flex justify-content-between">
    ...
    <label class="switch">
        <input type="checkbox" id="is_verified" name="is_verified" value="1" {{ $data->is_verified ? 'checked' : '' }}>
...
<script>
    $(document).ready(function(){
        $('#is_verified').change(function(){
            ...
            var userId = '{{ $data->id }}'; // تأكد من أن هذا المتغير يحتوي على ID المستخدم الصحيح

            $.ajax({
                url: '{{ route("admin.update-verification-status") }}', // المسار المناسب لتحديث الحالة
                type: 'POST',
                data: {
                    _token: '{{ csrf_token() }}',
                    id: userId,
