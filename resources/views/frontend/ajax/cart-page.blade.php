<!--==================== Cart Section Start ====================-->
<style>
/* اجعل الجدول قابلاً للتمرير الأفقي في الشاشات الصغيرة */
/* تخصيص شريط التمرير للمتصفحات الحديثة */
@media (max-width: 600px) {
    .cart-table {
        overflow-x: auto;
        scrollbar-color: silver transparent; /* لون الشريط: فضي، ولون الخلفية: شفاف */
        scrollbar-width: thin; /* تقليل عرض الشريط */
    }

    /* تخصيص شريط التمرير للمتصفحات التي تدعم Webkit مثل Chrome و Safari */
    .cart-table::-webkit-scrollbar {
        height: 8px; /* ارتفاع شريط التمرير */
    }

    .cart-table::-webkit-scrollbar-track {
        background: transparent; /* لون خلفية المسار */
    }

    .cart-table::-webkit-scrollbar-thumb {
        background-color: silver; /* لون شريط التمرير */
        border-radius: 10px; /* تدوير زوايا شريط التمرير */
        border: 2px solid transparent; /* إزالة الحواف حول الشريط */
    }
}
                ...
                <table class="shop_table cart">
                   ...
                   <tr class="woocommerce-cart-form__cart-item cart_item">
                      ...
                      <td class="product-name">
                        ...
                        <a href="{{ route('front.product', $product['item']['slug']) }}">{{ mb_strlen($product['item']['name_ar'],'UTF-8') > 35 ? mb_substr($product['item']['name_ar'],0,35,'UTF-8').'...' : $product['item']['name_ar']}}</a>
                        @elseif (app()->getLocale() === '17173493681JWu2KUR')
                              @if (!empty($product['item']['name_ku']))
                              <a href="{{ route('front.product', $product['item']['slug']) }}">{{ mb_strlen($product['item']['name_ku'],'UTF-8') > 35 ? mb_substr($product['item']['name_ku'],0,35,'UTF-8').'...' : $product['item']['name_ku']}}</a>
                              @else
                              <a href="{{ route('front.product', $product['item']['slug']) }}">{{ mb_strlen($product['item']['name_ar'],'UTF-8') > 35 ? mb_substr($product['item']['name_ar'],0,35,'UTF-8').'...' : $product['item']['name_ar']}}</a>
                              @endif
             ...
             <div class="cart-collaterals">
                <div class="cart_totals ">
                   ...
                   <div class="wc-proceed-to-checkout">
                      <a href="{{ route('front.checkout') }}" class="checkout-button">
