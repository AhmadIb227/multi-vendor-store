...
<style>
    .responsive-mobile-row-show{
     ...
     @media screen and (max-width: 775px) {
        ...
        .responsive-mobile-row-hide{
            display: none;
        }
        .checkout{
                padding: 3px 0px 70px !important;
        }
     }
</style>
...
<!-- Check Out Area Start -->
  <style>
    ...
    border-left: 20px solid #c9030f;
               ...
               .checkout-area .checkout-process li a.disabled::before {
    right: 880px;
     margin-left: 1000px;
    ...
    transition: all 0.3s ease-in;
               ...
               .checkout-process .nav-link.disabled {
    background-color: #e9ecef;
    color: #6c757d;
    border-radius: 14px;
    cursor: not-allowed;
    text-align: left; /* تغيير المحاذاة لليسار */
    padding-left: 20px; /* إضافة مساحة إلى اليسار */
}


               .checkout-process .nav-link.active {
    background-color: #e9ecef;
    color: #6c757d;
    border-radius: 14px;
    cursor: not-allowed;
    ...
    padding-left: 20px; /* إضافة مساحة إلى اليسار */
               ...
               .checkout-process .nav-link.active i {
                margin-left: -10px; /* يقلل المسافة بين الأيقونة والنص */
                margin-right: 35px; /* يضيف مسافة صغيرة بين الأيقونة والنص إذا لزم الأمر */
...
@media screen and (max-width: 775px) {
    .checkout-process {
        ...
        align-items: stretch;
        padding: 10px; /* تقليل المساحة الداخلية ليتناسب مع العرض */
    }
    ...
    .checkout-process .nav-item {
        width: 100%;
        margin-bottom: 10px; /* إضافة مسافة بين العناصر */
    }

    .checkout-process .nav-link {
        padding: 10px;
        font-size: 14px; /* تقليل حجم النص ليكون ملائماً للشاشة */
        display: flex;
        align-items: center;
    ...
    .checkout-process .nav-link span {
        font-size: 16px; /* تقليل حجم الرقم */
        margin-right: 10px;
    }

    .checkout-process .nav-link i {
    ...
    .checkout-process .nav-link.active, 
    .checkout-process .nav-link.disabled {
        background-color: #f8f9fa; /* لون الخلفية للعناصر النشطة والمعطلة */
        color: #495057; /* لون النص */
    }

    .checkout-process .nav-link.disabled {
        opacity: 0.6; /* تقليل التعتيم للعناصر المعطلة */
    }
}
...
<section class="checkout">
   <div class="container">
      <div class="row">
         <div class="col-lg-12 responsive-mobile-row-hide">
         ...
         <!-- ****************** responseve card checkout ********************** -->
          <div class="row p-3 responsive-mobile-row-show">
             <div class="col-4" style="color: #8c52fc ; font-weight: 600;">
                <span>1</span> 
                <span>
                   {{ __('Address') }}
                </span>
                           ...
                           <div class="content">
                              ...
                              <div class="billing-address">
                                 ...
                                 <div class="row">
                                    ...
                                    <style>
                                        /* تغيير لون شريط التمرير إلى الفضي */
select::-webkit-scrollbar {
    width: 12px;
}

select::-webkit-scrollbar-track {
    background: #f1f1f1; /* لون الخلفية للـ scrollbar */
}

select::-webkit-scrollbar-thumb {
    background-color: #c0c0c0; /* اللون الفضي */
    border-radius: 10px;
    border: 3px solid #f1f1f1; /* لون الحدود لتبدو بشكل أفضل */
}

/* For Firefox */
select {
    scrollbar-color: #c0c0c0 #f1f1f1; /* اللون الفضي */
                                 ...
                                 <div class="row">
                                    <div class="col-lg-12">
                                       <input type="text" id="Order_Note" class="form-control" name="order_notes"
                                          placeholder="{{ __('Order Note (Optional)') }})">
                                    </div>
                                 </div>
                              </div>
                              <div class="row">
    ...
    .d-flex.p-4.border.justify-content-between {
        flex-direction: column;
        align-items: flex-start;
        padding: 15px; /* تقليل المساحة الداخلية */
    }

    .d-flex.p-4.border.justify-content-between div {
    ...
    .mybtn1 {
        ...
        padding: 8px 12px; /* تقليل المساحة الداخلية للزر */
        width: 100%; /* جعل الزر يعرض بكامل العرض المتاح */
        text-align: center; /* توسيط النص داخل الزر */
        margin-top: 10px; /* إضافة مسافة أعلى وأسفل الزر */
    }

    h5.label {
        font-size: 16px; /* تقليل حجم عنوان الحقل */
        margin-bottom: 5px; /* تقليل المسافة أسفل العنوان */
    }

    .d-flex.p-4.border.justify-content-between div:last-child {
        margin-bottom: 0; /* إزالة المسافة السفلى لآخر عنصر */
    }

    /* تعديل تباعد النصوص */
    .d-flex.p-4.border.justify-content-between p {
        margin-bottom: 5px; /* تقليل المسافة بين الفقرات */
                                    ...
                                    <div class="d-flex p-4  border justify-content-between">
                                       <div class="">
                                          <p ><strong>@lang('Shop Name')</strong> : <strong>{{
                                                $vendor->shop_name }}</strong></p>
                                          <p ><strong>@lang('Shop Phone')</strong> : <strong>{{
                                                $vendor->phone }}</strong></p>
                                          <p ><strong>@lang('Shop Address')</strong> :
                              ...
                              <div class="row">
                                 <div class="col-lg-12 mt-3">
                                    <div class="bottom-area">
                                       <a href="javascript:;" id="step1-btn" class="mybtn1">{{ __('Back') }}</a>
                           ...
                           <div class="content">
                                     ...
                                     <div class="col-lg-12">
                                         <div class="nav flex-column" role="tablist" aria-orientation="vertical">
                                             ...
                                             <a class="nav-link payment" data-val="" data-show="{{$gt->showForm()}}"
                                                 ...
                                                 id="v-pills-tab{{ $gt->id }}-tab" data-toggle="pill" href="#v-pills-tab{{ $gt->id }}" role="tab"
                                                 ...
                                                 <p>
                                                     {{ $gt->title }}
                                                     @if ($gt->title != "Cash On Delivery")
                                                      <button class="copy-btn" onclick="copyToClipboard('{{$gt->details}}')"><i class="fa fa-copy"></i> انسخ رقم المحفظة  </button>
                                                     @endif                                             
                                             ...
                                             @if (auth()->check())
                                             {{-- <a class="nav-link payment" href="javascript:;" data-show="no" data-val="{{ $gt->keyword }}"
                                                 data-toggle="pill" role="tab" data-form="{{ route('front.wallet.submit') }}"
                                                 aria-controls="v-pills-tab{{ $gt->id }}" aria-selected="false">
                                                 <div class="icon">
                                                     <span class="radio"></span>
                                                 </div>
                                             ...
                                             {{-- <button class="copy-btn" onclick="copyToClipboard('v-pills-tab{{ $gt->id }}-tab')">Copy ID</button> --}}
                                             @endif
                                         </div>
                                     </div>
                                     <div class="col-lg-12">
                             ...
                             <script>
                                function copyToClipboard(id) {
                                    ...
                                    let valueToCopy = id.replace(prefix, ''); 
                            ...
                            //  }
                              ...
                              <div class="row">
                                 <div class="col-lg-12 mt-3">
                                    <div class="bottom-area">
                                       <a href="javascript:;" id="step2-btn" class="mybtn1">{{ __('Back') }}</a>
                  ...
                  <div class="packeging-area">
                     ...
                     <div class="radio-design">
                        <input type="radio" class="packing" data-price="{{ round($data->price * $curr->value,2) }}"
                           data-form="{{$data->title_en}}" id="free-package{{ $data->id }}" name="packeging_id"
                           value="{{ $data->id }}" {{ ($loop->first) ? 'checked' : '' }}>
                        <span class="checkmark"></span>
