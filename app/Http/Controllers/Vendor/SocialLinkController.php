    ...
    public function api_delete($id)
{
    ...
    {
        $datas = SocialLink::latest('id')->get();
        
        return Datatables::of($datas)
            ->addColumn('status', function(SocialLink $data) {
                return $data->status == 1 ? 'Activated' : 'Deactivated';
            })
            ->addColumn('action', function(SocialLink $data) {
                return [
                    'edit' => route('vendor-sociallink-edit', $data->id),
                    'delete' => route('vendor-sociallink-delete', $data->id),
                ];
            })
            ->toJson();
    }
