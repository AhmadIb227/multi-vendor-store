...
class DeliveryController extends VendorBaseController
{
    ...
    public function api_datatables()
    {
        $user = $this->user;
    ...
    if (isset($vendor_shipping->$user_id)) {
        ...
        // التعامل مع الحالة عندما يكون $vendor_shipping أو $user_id غير موجود
    }

    $vendor_packing_id = json_decode($order->vendor_packing_id);
    if (isset($vendor_packing_id->$user_id)) {
        ...
        // التعامل مع الحالة عندما يكون $vendor_packing_id أو $user_id غير موجود
    }
}
