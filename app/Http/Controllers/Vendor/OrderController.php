...
class OrderController extends VendorBaseController
{
    ...
    public function api_orders(Request $request)
    {
        ...
        $datas = Order::with(['vendororders' => function ($query) use ($user) {
            ...
            return $item->vendororders()->where('user_id', '=', $user->id)->count() == 0;
        });
        ...
        return DataTables::of($datas)
            ...
            ->editColumn('pay_amount', function (Order $data) use ($user) {
                ...
                if ($data->is_shipping == 1) {
                    $vendor_shipping = json_decode($data->vendor_shipping_id);
                    if (isset($vendor_shipping->{$user->id})) {
                        $shipping_id = $vendor_shipping->{$user->id};
                        $shipping = Shipping::find($shipping_id);
                        if ($shipping) {
                            $price += round($shipping->price * $data->currency_value, 2);
                        }
                    }
    
                    $vendor_packing_id = json_decode($data->vendor_packing_id);
                    if (isset($vendor_packing_id->{$user->id})) {
                        $packing_id = $vendor_packing_id->{$user->id};
                        $packaging = Package::find($packing_id);
                        if ($packaging) {
                            $price += round($packaging->price * $data->currency_value, 2);
                        }
                    }
                }
            ...
            ->addColumn('action', function (Order $data) use ($user) {
                $pending = $data->vendororders()->where('user_id', '=', $user->id)->where('status', 'pending')->count() > 0 ? "selected" : "";
                $processing = $data->vendororders()->where('user_id', '=', $user->id)->where('status', 'processing')->count() > 0 ? "selected" : "";
                $completed = $data->vendororders()->where('user_id', '=', $user->id)->where('status', 'completed')->count() > 0 ? "selected" : "";
                $declined = $data->vendororders()->where('user_id', '=', $user->id)->where('status', 'declined')->count() > 0 ? "selected" : "";
    
                return '
                    <div class="action-list">
                        <a href="' . route("vendor-order-show", $data->order_number) . '" class="btn btn-primary product-btn"><i class="fa fa-eye"></i>  ' . __("Details") . ' </a>
                        <select class="vendor-btn ' . $data->vendororders()->where('user_id', '=', $user->id)->first()->status . ' ">
        ...
        return Datatables::of($datas)
            ...
            ->editColumn('pay_amount', function (Order $data) {
                ...
                if ($order->is_shipping == 1) {
                    ...
                    // shipping cost
                    if ($vendor_shipping && property_exists($vendor_shipping, $user_id)) {
...
if ($vendor_packing_id && property_exists($vendor_packing_id, $user_id)) {
