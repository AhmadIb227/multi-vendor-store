...
class FrontendController extends FrontBaseController
{
    ...
    // {
    //     Session::put('language_rtl', DB::table('languages')->find($id)->rtl);
...
{
    if ($id === null) {
        $language = DB::table('languages')->where('language', 'العربية')->first();
    } else {
...
{
    // تحقق من تعيين اللغة في الجلسة، إذا لم تكن معينة، قم بتعيين اللغة الافتراضية (العربية)
    if (!Session::has('language')) {
       $defaultLanguage = \App\Models\Language::where('language', 'العربية')->first();
        Session::put('language_rtl', $defaultLanguage->rtl);
        Session::put('language', $defaultLanguage->id);
    }
    ...
    $data['products'] =  Product::where('status', 1)->paginate(8);
...
{
    // تحميل المنتجات مع Pagination حسب الصفحة المطلوبة
    $products = Product::where('status', 1)->paginate(8);

    if ($request->ajax()) {
        // عرض المنتجات في جزء HTML مخصص
        $view = view('frontend.partials.products', compact('products'))->render();
        return response()->json(['html' => $view, 'hasMoreProducts' => $products->hasMorePages()]);
    }
    ...
    public function contact()
    {
       ...
       public function contactadminemail(Request $request)
    {
        $name = $request->name;
        $email = $request->email;
        $phone = $request->phone;
        $messageText = $request->text;
        $x="youwithmecompany@gmail.com";
        try {
            Mail::to($x)->send(new ContactEmail($name, $email, $phone, $messageText));
            
            return response('تم إرسال الرسالة بنجاح!');
        } catch (\Exception $e) {
            return redirect()->route('front.contact')->with('error', 'حدث خطأ أثناء إرسال الرسالة. حاول مرة أخرى.');
        }
    }
    ...
    public function contactvender()
    {
        ...
        $ps = $this->ps;
        return view('frontend.contactvender', compact('ps'));
    }
    
    public function contactvenderemail(Request $request)
    {
         $VendorEmail = $request->vender_email;
        ...
        try {
            Mail::to($VendorEmail)->send(new ContactEmail($name, $email, $phone, $messageText));
    ...
    public function aboutUs()
    {
        ...
        $ps = $this->ps;
        return view('frontend.about', compact('ps'));
    }
}
