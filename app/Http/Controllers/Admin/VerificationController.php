...
class VerificationController extends AdminBaseController
{
    ...
    public function status($id1,$id2)
    {
        ...
        Log::info('Verification status updated.', ['id' => $id1, 'status' => $id2]);
        ...
        if ($id2 === 'Verified') {
            ...
            if ($user && $user->referral_link) {
                Log::info('User with referral link found.', ['user_id' => $user->id, 'referral_link' => $user->referral_link]);
    
                
                $referrer = User::where('referral_link', $user->referral_link)->first();
    
                if ($referrer) {
                    Log::info('Referrer found and points added.', ['referrer_id' => $referrer->id]);
                   
                    // $referrer->increment('points_vendor', 1);
                } else {
                    Log::warning('Referrer not found for referral link.', ['referral_link' => $user->referral_link]);
                }
            } else {
                Log::warning('No referral link found for user.', ['user_id' => $user->id]);
            }
        }
