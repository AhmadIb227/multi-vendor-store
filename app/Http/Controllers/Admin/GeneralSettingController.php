...
//     public function vendor_color_self()
// {
//     return view('admin.generalsetting.vendor_color_self');
// }

// public function updateGeneralSettings(Request $request)
// {
//     $request->validate([
//         'vendor_color' => 'nullable|string',
//     ]);

//     $gs = Generalsetting::find(1); // افترض أن هناك سجل واحد فقط في generalsettings
//     $gs->vendor_color = $request->vendor_color;
//     $gs->save();

//     return redirect()->back()->with('success', 'Vendor store default background color updated successfully.');
// }
    ...
    public function generalupdate(Request $request)
    {
        ...
        if (!empty($request->colors)) {
            $input['colors'] = $request->input('colors');
        }
            ...
            $input['vendor_color'] = $request->input('vendor_color', $data->vendor_color);
    ...
    {    
        $rules = [
            'photo' => 'nullable|mimes:jpeg,jpg,png,svg|max:2048', // التحقق من الصورة إذا كانت موجودة
        ];
    
        $validator = FacadesValidator::make($request->all(), array_merge($this->rules, $rules));
        ...
        // التعامل مع إعدادات معينة
        $input['vendor_ship_info'] = $request->vendor_ship_info ?: 0;
        $input['instamojo_sandbox'] = $request->instamojo_sandbox ?: 0;
        $input['paypal_mode'] = $request->paypal_mode ?: 'live';
        $input['paytm_mode'] = $request->paytm_mode ?: 'live';
    
        if ($request->hasFile('photo')) {
            $image = $request->file('photo');
            $imageName = \PriceHelper::ImageCreateName($image); // إنشاء اسم جديد للصورة
    
            // ضغط الصورة باستخدام مكتبة Intervention Image
            $img = Image::make($image->getRealPath());
            $img->resize(800, 800, function ($constraint) {
                $constraint->aspectRatio();
            })->save(public_path('assets/images/generalsettings/') . $imageName);
    ...
    public function generalupdatevendor(Request $request)
  {
    ...
    // تحديث الألوان
    if (!empty($request->vendor_color)) {
        $input['vendor_color'] = $request->input('vendor_color');

       
        User::where('is_vendor', 2)
            ->whereNull('background_color')
            ->update(['background_color' => $request->vendor_color]);
    }
