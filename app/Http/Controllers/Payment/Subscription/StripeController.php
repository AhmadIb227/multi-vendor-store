    ...
    public function store(Request $request)
{
    ...
    // جلب تفاصيل الاشتراك المحدد
    $subs = Subscription::findOrFail($request->subs_id);
    $user = $this->user;

    // حساب المبلغ الإجمالي للاشتراك
    $item_amount = $subs->price * $this->curr->value;

    // إعداد البيانات المتعلقة بالاشتراك
    $sub['user_id'] = $user->id;
    $sub['subscription_id'] = $subs->id;
    $sub['title'] = $subs->title;
    $sub['currency_sign'] = $this->curr->sign;
    ...
    try {
        ...
        // تخزين بيانات الاشتراك في الجلسة
        Session::put('subscription_data', $sub);
        ...
        // في حالة حدوث خطأ، الرجوع مع رسالة خطأ
        return back()->with('unsuccess', $e->getMessage());
    }
}
